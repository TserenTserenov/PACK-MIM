...
| Force | Tension |
|-------|---------|
| Накопленный scope ↔ размер отдельного chunk | Каждый следующий chunk включает scope всех предыдущих ради generalisation, но это же наращивание держит объём работы каждого шага растущим — метод балансирует между выигрышем от обобщения паттернов и стоимостью удержания всё большего scope в одном проекте |
| Temporary interfaces между chunks ↔ давление их не убирать | Chunk #1 подключается через temporary interface (шаг 3), который по замыслу временный, но по мере роста системы удаление уже работающего интерфейса требует отдельного усилия, которое легко отложить до «финализации на последнем chunk» |
| Промежуточная ephemeral-документация ↔ ощущение завершённости прототипа | Каждый chunk кроме финального — прототип с ephemeral-документацией (шаг 6), но команда, увидевшая работающий chunk в проде, естественно воспринимает его как завершённый результат, а не временный шаг к обобщению |
...
|------|--------------------------|
| Generalisation между chunks переоценивается там, где паттерны на самом деле не совпадают | Кумулятивный подход обещает выигрыш именно через обнаружение общих паттернов между chunks, но менеджер, уже вложившийся в идею объединения scope, рискует форсировать generalisation там, где chunks реально независимы — искусственное обобщение создаёт сложность вместо её снижения |
| Temporary interface остаётся постоянным, потому что переход к следующему chunk важнее | Внимание команды естественно смещается на запуск следующего chunk (видимый прогресс), а не на снятие временного интерфейса предыдущего — временное решение получает шанс закрепиться просто потому, что убрать его никогда не становится приоритетом момента |
