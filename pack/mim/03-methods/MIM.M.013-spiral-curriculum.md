...
| Force | Tension |
|-------|---------|
| Углубление ↔ доступность повторного захода | Каждый виток обязан быть глубже предыдущего (принцип 2 Bruner), но резкий скачок Bloom-глубины без прочной опоры на предыдущий виток рискует превратить возврат к теме в непосильную задачу вместо укрепления понимания |
| Ротация домена ↔ узнаваемость принципа | Правило R6 требует менять домен на каждом витке (не повторять контекст) ради транс-дисциплинарности, но незнакомый домен добавляет когнитивную нагрузку сверх самого принципа — ученик тратит внимание на распознавание нового контекста, а не на углубление понимания принципа |
| Явный bridge ↔ самостоятельное узнавание связи | Метод требует произносить bridge-фразу явно («Помнишь, как мы…?»), что гарантирует видимую преемственность, но также забирает у ученика шанс самому обнаружить связь между витками — а именно самостоятельное обнаружение сильнее закрепляет transfer |
...
|------|--------------------------|
| Домен по умолчанию — самый удобный, не самый непохожий | Правило R6 явно требует менять домен, но при проектировании витка внимание тяготеет к домену, который проще всего сформулировать (чаще — тот же, что уже использовался), а не к домену, который даёт максимальный transfer-эффект через контраст |
| Виток проектируется по глубине задания, не по факту готовности ученика | Планирование естественно опирается на заранее заданную Bloom-цель следующего витка (глубина+1), а фактический замер «может ли ученик выполнить can-do утверждения» (шаг 9 алгоритма) рискует превратиться в формальность после того, как виток уже спроектирован и проведён — оценка подтверждает план, а не переопределяет его |
